    ...
    bmak = bmak || {
        ...
        bpd: function () {
            ...
            try {
                ...
                if (void 0 !== y[1]) {
                    var M = y[1];
                    void 0 !== bmak.mn_r[M] && (C = bmak.mn_r[M])
                }
                if (void 0 !== y[2]) {
                    var S = y[2];
                ...
                var L = "";
                try {
                    t.stack && "string" == typeof t.stack ? L = t.stack.replace(/\"/g, "\\'") : "string" == typeof t && (L = t.replace(/\"/g, "\\'")), L = L.slice(0, 1e3), bmak.sd_debug(",s2:" + L), bmak.sensor_data = bmak.ver + "-1,2,-94,-100," + bmak.uar() + "-1,2,-94,-120," + L
                } catch (t) {
                    t.stack && "string" == typeof t.stack ? L = t.stack.replace(/\"/g, "\\'") : "string" == typeof t && (L = t.replace(/\"/g, "\\'")), L = L.slice(0, 1e3), bmak.sd_debug(",s3:" + L), bmak.sensor_data = bmak.ver + bmak.sensor_data + ",s3:" + L
                }
            }
            try {
                var P = bmak.od(bmak.cs, bmak.api_public_key).slice(0, 16),
                    T = Math.floor(bmak.get_cf_date() / 36e5),
                    F = bmak.get_cf_date(),
                    D = P + bmak.od(T, P) + bmak.sensor_data;
                bmak.sensor_data = D + ";" + (bmak.get_cf_date() - t) + ";" + bmak.tst + ";" + (bmak.get_cf_date() - F)
            } catch (t) {}
            bmak.sd_debug("</bpd>")
        },
        od: function (t, a) {
        ...
        mn_poll: function () {
            if (0 == bmak.mn_state) {
                ...
                null != a && (bmak.mn_update_challenge_details(a), bmak.mn_sen && (bmak.mn_state = 1, bmak.mn_mc_indx = 0, bmak.mn_al = [], bmak.mn_il = [], bmak.mn_tcl = [], bmak.mn_lg = [], bmak.mn_rts = bmak.get_cf_date(), bmak.mn_rt = bmak.mn_rts - bmak.start_ts, setTimeout(bmak.mn_w, bmak.mn_tout)))
            }
        },
        ...
        mn_s: function (t) {
            ...
            for (var v = 0; v < l; v++) {
                for (var h, g = new Array(64), w = e, y = n, C = o, E = m, j = r, h = i, M = c, S = b, _ = 0; _ < 64; _++) {
                    var x, A, L, P, T, F;
                    _ < 16 ? g[_] = u[v][_] : (x = bmak.rotate_right(g[_ - 15], 7) ^ bmak.rotate_right(g[_ - 15], 18) ^ g[_ - 15] >>> 3, A = bmak.rotate_right(g[_ - 2], 17) ^ bmak.rotate_right(g[_ - 2], 19) ^ g[_ - 2] >>> 10, g[_] = g[_ - 16] + x + g[_ - 7] + A), A = bmak.rotate_right(j, 6) ^ bmak.rotate_right(j, 11) ^ bmak.rotate_right(j, 25), L = j & h ^ ~j & M, P = S + A + L + a[_] + g[_], x = bmak.rotate_right(w, 2) ^ bmak.rotate_right(w, 13) ^ bmak.rotate_right(w, 22), T = w & y ^ w & C ^ y & C, F = x + T, S = M, M = h, h = j, j = E + P >>> 0, E = C, C = y, y = w, w = P + F >>> 0
                }
                e += w, n += y, o += C, m += E, r += j, i += h, c += M, b += S
            }
